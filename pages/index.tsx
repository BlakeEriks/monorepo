...
export default function Home() {
  ...
  const fetchTopics = async () => {
    try {
      setLoading(true)
      const response = await fetch('/api/newsletter/topics?status=VOTING')
  ...
  useEffect(() => {
    ...
    if (vote && !voteRecorded) {
      // Handle vote
      handleVote(vote as string)
    }
  }, [router.isReady, router.query, voteRecorded])

  const handleVote = async (topicId: string) => {
    try {
      // Generate a simple hash from timestamp + random number as a basic anonymous identifier
      const emailHash = `anon-${Date.now()}-${Math.random().toString(36).substring(2, 15)}`
      ...
      const response = await fetch('/api/newsletter/vote', {
        ...
        body: JSON.stringify({
          topicId,
          emailHash,
          // If there is a subscriberEmail, send it as well
    ...
    >
      <main className='flex flex-col gap-8 row-start-2 items-center sm:items-start max-w-lg w-full'>
        ...
        <div className='w-full'>
          <h2 className='text-xl font-semibold mb-4'>Current Voting Topics</h2>
          {loading ? (
            <p>Loading topics...</p>
          ) : topics.length > 0 ? (
            topics.map((topic, i) => (
            ...
            <p>No voting topics available.</p>
          )}
        </div>
